...
public class ShieldDamageDivided : Effect
{
    public override IEnumerator Process(GameObject caster, CardEffectsController effectController, List<GameObject> target, Card card, int effectIndex)
    {
        ...
        foreach (GameObject t in target)
            new EffectFactory().GetEffect(Card.EffectType.ShieldDamage).Process(caster, effectController, new List<GameObject> { t }, card, effectIndex);
    ...
    public override int GetSimulatedVitDamage(GameObject caster, CardEffectsController effectController, List<GameObject> target, Card card, int effectIndex)
    {
        ...
        card.SetTempEffectValue(Mathf.CeilToInt(card.effectValue[effectIndex] / (float)target.Count));
        foreach (GameObject t in target)
            output += new EffectFactory().GetEffect(Card.EffectType.ShieldDamage).GetSimulatedVitDamage(caster, effectController, new List<GameObject> { t }, card, effectIndex);
    ...
    public override int GetSimulatedShieldDamage(GameObject caster, CardEffectsController effectController, List<GameObject> target, Card card, int effectIndex)
    {
        ...
        card.SetTempEffectValue(Mathf.CeilToInt(card.effectValue[effectIndex] / (float)target.Count));
        foreach (GameObject t in target)
            output += new EffectFactory().GetEffect(Card.EffectType.ShieldDamage).GetSimulatedShieldDamage(caster, effectController, new List<GameObject> { t }, card, effectIndex);
